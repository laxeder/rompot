        ...
        this.wa.sock.ev.on("connection.update", (update) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (update.connection == "open") {
                    ...
                    yield this.wa.readChats();
                    yield this.wa.readUsers();
                    yield this.wa.readPolls();
                    yield this.wa.readSendedMessages();
                ...
                if (update.connection == "close") {
                    ...
                    if (status == baileys_1.DisconnectReason.restartRequired) {
                        return this.connectionResolve(yield this.wa.reconnect(false));
                    }
                    setTimeout(() => __awaiter(this, void 0, void 0, function* () { return this.connectionResolve(yield this.wa.reconnect(botStatus != "online")); }), 1000);
                }
            }
            catch (err) {
                this.wa.ev.emit("error", err);
            ...
            for (const update of updates) {
                try {
                    ...
                    if (((_a = this.wa.users[update.id]) === null || _a === void 0 ? void 0 : _a.name) != update.notify || update.verifiedName) {
                        yield this.wa.readUser(update, true);
                    }
                }
                catch (err) {
                    this.wa.ev.emit("error", err);
            ...
            for (const update of updates) {
                try {
                    ...
                    if (!this.wa.chats[update.id]) {
                        ...
                        this.wa.chats[update.id].users[this.wa.id] = new User_1.default(this.wa.id);
            ...
            for (const update of updates) {
                try {
                    ...
                    if (((_a = this.wa.chats[update.id]) === null || _a === void 0 ? void 0 : _a.name) != update.subject) {
                        yield this.wa.readChat(update, true);
                    }
                }
                catch (err) {
                    this.wa.ev.emit("error", err);
