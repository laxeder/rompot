...
class ConfigWAEvents {
    ...
    configHistorySet() {
        this.wa.sock.ev.on("messaging-history.set", (update) => __awaiter(this, void 0, void 0, function* () {
            const readed = [];
            yield Promise.all(update.chats.map((chat) => __awaiter(this, void 0, void 0, function* () {
                try {
                    if (!chat.id.includes("@g") || readed.includes(chat.id))
                        return;
                    readed.push(chat.id);
                    yield this.wa.readChat(chat);
                }
                catch (err) {
                    this.wa.ev.emit("error", err);
                }
            })));
            yield Promise.all(update.contacts.map((user) => __awaiter(this, void 0, void 0, function* () {
                try {
                    if (!user.id.includes("@s") || readed.includes(user.id))
                        return;
                    readed.push(user.id);
                    yield this.wa.readUser(user);
                }
                catch (err) {
                    this.wa.ev.emit("error", err);
                }
