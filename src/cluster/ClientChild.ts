...
export type ClientChildConfig = { maxTimeout: number } & ConnectionConfig;

export default class ClientChild<Bot extends IBot> extends ClientEvents {
  ...
  /** Configuração */
  public config: ClientChildConfig;
  /** Bot */
  public bot: Bot;
   ...
   */
  public static getClients(): Record<string, ClientChild<IBot>> {
   ...
   */
  public static saveClients(clients: Record<string, ClientChild<IBot>>): void {
   ...
   */
  public static getClient(id: string): ClientChild<IBot> {
    const clients = ClientChild.getClients();
   ...
   */
  public static saveClient(client: ClientChild<IBot>): void {
    if (!global.hasOwnProperty("rompot-clients-cluster") || typeof global["rompot-clients-cluster"] != "object") {
      global["rompot-clients-cluster"] = {};
    }
  ...
  public static generateId(): string {
    return `${process.pid}-${Date.now()}-${Object.keys(ClientChild.getClients()).length}`;
  }
}
